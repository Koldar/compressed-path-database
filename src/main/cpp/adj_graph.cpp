...
vector<OutArc>::const_iterator AdjGraph::arcBegin(int v) const {
	return out_arc.begin() + out_begin[v];
}

vector<OutArc>::const_iterator AdjGraph::arcEnd(int v) const {
...
OutArc& AdjGraph::getArc(int v, int w) {
	...
	throw CpdException{"arc from %d and %d not found", v, w};
}
...
bool AdjGraph::containsArc(dpf::nodeid_t source, dpf::nodeid_t sink) const {
...
dpf::big_integer AdjGraph::getTotalNumberOfArcs() const {
	return static_cast<dpf::big_integer>(this->out_arc.size());
}
